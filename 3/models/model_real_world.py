...
from models.basis import ResBlk, conv1x1, conv3x3
...
class RGB2GRAY(nn.Module):
	...
	def forward(self, cont, cont_feat, proxy, cont_gray, real):
		...
		#b_grid = self.actv(b_grid)
		#guide = self.guide(cont)
		guide = cont_gray
		slice_coeffs = self.slice(b_grid, guide)
		out = self.apply_coeffs(slice_coeffs, real)
