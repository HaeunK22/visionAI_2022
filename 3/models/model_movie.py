...
class Bilateral_grid(nn.Module):
	def __init__(self, dims):
		...
		self.F = nn.Conv2d(256, 64, 1, padding=0, padding_mode='reflect')
	...
	def forward(self, x, sa, sb):
		...
		L2 = F.interpolate(L2, (64, 64), mode='nearest')
		L3 = F.interpolate(L3, (64, 64), mode='nearest')
...
class ConvBlock(nn.Module):
	def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, use_bias=True, activation=nn.PReLU, batch_norm=False):
...
class RGB2GRAY(nn.Module):
	...
	def forward(self, cont, cont_feat, proxy, frames, cont_gray, frame_gray):
		cont_feat = self.mapping(cont_feat)
		style_feat = self.mapping(proxy)
		b_grid = self.bilateral_grid(cont, cont_feat, style_feat)
		b_grid_cont = b_grid.reshape(b_grid.shape[0], 4, -1, b_grid.shape[-2], b_grid.shape[-1])
		b_grid_interpolate = b_grid.reshape(1,1,b_grid.shape[0], -1)
		b_grid_interploate = torch.nn.functional.interpolate(b_grid_interpolate, [frames.shape[0], b_grid_interpolate.shape[-1]], mode='bilinear').squeeze()
		b_grid_interploate = b_grid_interploate.reshape(frames.shape[0], 4, -1, b_grid.shape[-2], b_grid.shape[-1])
		guide = cont_gray
		g_frames = frame_gray
		slice_coeffs = self.slice(b_grid_cont, guide)
		s_frames = self.slice(b_grid_interploate, g_frames)
		out = self.apply_coeffs(slice_coeffs, cont)
		out_movie = self.apply_coeffs(s_frames, frames)

		return out, out_movie